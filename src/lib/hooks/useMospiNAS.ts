...
/**
 * Hook to load pre-fetched MoSPI NAS quarterly GVA by sector data.
 * Source: nas-gva-sectors.json generated by scripts/generate-nas-gva.mjs
 */
export function useNASGVASectorData() {
  return useQuery<NASGVASectorData>({
    queryKey: ["mospi", "nas", "gva-sectors"],
    queryFn: async () => {
      const url = `${BASE_PATH}/data/mospi/nas-gva-sectors.json`;
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error("NAS GVA sector data not available");
      }

      const text = await response.text();
      try {
        return JSON.parse(text) as NASGVASectorData;
      } catch {
        throw new Error("NAS GVA sector data returned invalid format");
      }
    },
    staleTime: Infinity,
...
/**
 * Convert GVA sector data into an array of DataSeries (one per sector key).
 * Skips null/base-year entries automatically.
 */
export function nasGVAToSeries(
  data: NASGVASectorData,
  sectorKeys: GVASectorKey[]
): DataSeries[] {
  return sectorKeys.map((key) => ({
    source: "mospi" as const,
    indicator: data.sectorLabels[key] ?? key,
    indicatorId: `NAS_GVA_${key.toUpperCase()}`,
    unit: "%",
